...
# Main digital model implementation module
...
def main(k, v, n):
    ...
    # Setting the obj. fun.
    model.setObjective(sum(y[k] for k in K), GRB.MINIMIZE) # Mathematical expression (3)
    ...
    model.addConstrs(sum(q[k, j] for k in K) >= x[j] for j in N)  # Constraint (8)
    ...
    model.addConstrs((1-u_slow[k, j, f])*big_M + u_slow[k, j, f] * charging_slots_slow[f] >= (tau[k] + t[k, j]) * q[k, j] for k in K for j in N1 for f in F1) # Constraint (15)
    model.addConstrs((1-u_fast[k, j, f])*big_M + u_fast[k, j, f] * charging_slots_fast[f] >= (tau[k] + t[k, j]) * q[k, j] for k in K for j in N2 for f in F2) # Constraint (16)
    model.addConstrs(-(1-u_slow[k, j, f])*big_M + u_slow[k, j, f] * charging_slots_slow[f] <= (P1k[k] + t[k, j]) * q[k, j] for k in K for j in N1 for f in F1) # Constraint (17)
    model.addConstrs(-(1-u_fast[k, j, f])*big_M + u_fast[k, j, f] * charging_slots_fast[f] <= (P2k[k] + t[k, j]) * q[k, j] for k in K for j in N2 for f in F2) # Constraint (18)
    ...
    if solution_status == GRB.OPTIMAL:
        ...
        print("tau:", tau)
        print("After charging time limit (SLOW):", P1k)
        print("After charging time limit (FAST):", P2k)
        ...
        # This should be commented out if you are trying to run start_experiments.py
        ...
        for name, val in zip(names, values):
            if val != 0:
                print(f"{name} = {val}")
        print("\r")
        print("I SOLVED a problem instance with " + str(k) + " busses, " + \
              str(v) + " depots, and " + str(n) + " charging options.")
    else:
        print("\r")
        print("Infeasible problem !")
        print("I TRIED TO SOLVE a problem instance with " + str(k) + " busses, " + \
              str(v) + " candidate locations, and " + str(n) + " charging options.")
